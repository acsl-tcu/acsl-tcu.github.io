...
import { CRUDState, CRUDInfo } from './types';
  ...
  switch (info.type) {
    case 'push_C': {
      ...
      if ('files' in info.Data) {
        ...
        const filtered = info.Data.filter((v) => tmp.C.includes(v.id));
    ...
    case 'push_R': {
      const data = await fetchData(route, info);
      info.set?.(data);
    ...
    case 'push_U': {
      ...
      const onServer = await fetchData(route, info);
      const Uid = tmp.U.filter((v) => onServer.some((s) => s.id === v));
      const targetU = info.Data.filter((v) => Uid.includes(v.id));
